...
void colorCombo(double msTime, uint32_t color1, uint32_t color2, uint32_t color3, uint32_t color4 ){
  if (millis() % (int)(msTime*4) < msTime){
    loopOne(msTime, color1, noColor);
  }
  else if (millis() % (int)(msTime*4) < 2*msTime){
    loopOne(msTime, color2, noColor);
  }
  else if (millis() % (int)(msTime*4) < 3*msTime){
    loopOne(msTime, color3, noColor);
  }
  else{
    loopOne(msTime, color4, noColor);
  }
}
